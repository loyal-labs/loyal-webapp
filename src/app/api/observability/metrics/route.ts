...
import { reportBrowserLoadingMetricEnvelope } from "@/features/observability/server";
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    const envelope = parseBrowserLoadingMetricEnvelope(
      await readJsonBody(request)
    );
    await reportBrowserLoadingMetricEnvelope(envelope);
